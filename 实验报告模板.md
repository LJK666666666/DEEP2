...
**原始 Anchor 模型最终评估结果 (119k 迭代, mAP = 71.18%)**：
```
mAP: 0.7118
aeroplane       : 0.7853
bicycle         : 0.7973
...
**K-means++ Anchor 模型最终评估结果 (119k 迭代, mAP = 70.80%)**：
```
mAP: 0.7080
aeroplane       : 0.7669
bicycle         : 0.7906
...
共解析 5011 张图像，15662 个目标框。

| 宽高比区间 | 数量 | 占比 | 说明 |
|-----------|------|------|------|
| [0, 0.5) | 5114 | 32.7% | 竖长形目标（如人站立） |
| [0.5, 1.0) | 6065 | 38.7% | 接近正方形偏高 |
| [1.0, 2.0) | 3769 | 24.1% | 接近正方形偏宽 |
| [2.0, 3.0) | 526 | 3.4% | 横长形目标（如汽车侧面） |
| [3.0, +∞) | 188 | 1.2% | 极端横长形 |
...
**K-means++ 优化配置：**
```yaml
MIN_SIZES: [31, 75, 115, 166, 181, 255]
MAX_SIZES: [48, 93, 138, 173, 215, 306]
```

**聚类中心详情（归一化坐标）：**

| Anchor | Width | Height | 宽高比 | 说明 |
|--------|-------|--------|-------|------|
| 1 | 0.0850 | 0.1252 | 0.68 | 小目标，偏竖 |
| 2 | 0.2072 | 0.3001 | 0.69 | 中小目标，偏竖 |
| 3 | 0.2682 | 0.5493 | 0.49 | 中目标，竖长 |
| 4 | 0.6924 | 0.4410 | 1.57 | 大目标，偏横 |
| 5 | 0.4458 | 0.8153 | 0.55 | 大目标，竖长 |
| 6 | 0.8668 | 0.8348 | 1.04 | 超大目标，接近正方 |

### 5. 实验效果对比

| 指标 | 默认 Anchor | K-means++ Anchor | 差异 |
|------|------------|------------------|------|
| 聚类平均 IoU | - | 0.5962 | - |
| **mAP@0.5 (119k迭代)** | **0.7118 (71.18%)** | **0.7080 (70.80%)** | **-0.38%** |

#### 各类别 AP 详细对比 (119k 迭代最终结果)

| 类别 | 默认 Anchor | K-means++ | 差异 | 优势方 |
|------|------------|-----------|------|--------|
| aeroplane | 0.7853 | 0.7669 | -0.0184 | 默认 |
| bicycle | 0.7973 | 0.7906 | -0.0067 | 默认 |
| bird | 0.6895 | 0.6902 | +0.0007 | K-means++ |
| boat | 0.6103 | 0.6115 | +0.0012 | K-means++ |
| bottle | 0.4651 | 0.4359 | -0.0292 | 默认 |
| bus | 0.7548 | 0.7828 | +0.0280 | K-means++ |
| car | 0.7975 | 0.7877 | -0.0098 | 默认 |
| cat | 0.8364 | 0.8256 | -0.0108 | 默认 |
| chair | 0.5001 | 0.5125 | +0.0124 | K-means++ |
| cow | 0.7553 | 0.7413 | -0.0140 | 默认 |
| diningtable | 0.6645 | 0.6597 | -0.0048 | 默认 |
| dog | 0.8265 | 0.8162 | -0.0103 | 默认 |
| horse | 0.8095 | 0.8388 | +0.0293 | K-means++ |
| motorbike | 0.7973 | 0.7888 | -0.0085 | 默认 |
| person | 0.7547 | 0.7538 | -0.0009 | 默认 |
| pottedplant | 0.4394 | 0.4491 | +0.0097 | K-means++ |
| sheep | 0.7325 | 0.7049 | -0.0276 | 默认 |
| sofa | 0.7177 | 0.7148 | -0.0029 | 默认 |
| train | 0.7942 | 0.7882 | -0.0060 | 默认 |
| tvmonitor | 0.7082 | 0.7003 | -0.0079 | 默认 |

**K-means++ 优于默认 Anchor 的类别（6个）**：bird, boat, bus, chair, horse, pottedplant

**默认 Anchor 优于 K-means++ 的类别（14个）**：aeroplane, bicycle, bottle, car, cat, cow, diningtable, dog, motorbike, person, sheep, sofa, train, tvmonitor
...
1. **实验结果分析**：
   - 默认 Anchor 最终 mAP = **71.18%** (119k 迭代)
   - K-means++ Anchor 最终 mAP = **70.80%** (119k 迭代)
   - 默认 Anchor 略优于 K-means++ 优化 Anchor（差距 0.38%）

2. **结果解读**：
   - SSD 原始论文中设计的 Anchor 尺寸经过精心调优，对于 VOC 数据集已经接近最优
   - K-means++ 在部分类别上有改进（如 horse +2.93%, bus +2.80%, chair +1.24%）
   - 但在更多类别上略有下降，导致整体 mAP 略低

3. **K-means++ 的适用场景**：
   - 当目标数据集与 COCO/VOC 差异较大时，K-means++ 优化更有价值
   - 对于特定行业数据集（如医疗影像、工业检测），聚类优化往往效果显著
   - 对于已经被广泛研究的通用数据集，默认参数通常已经很好

4. **实践建议**：
   - 对于全新数据集，建议使用 K-means++ 进行 Anchor 优化
   - 聚类数量应与特征图层数匹配
   - 需要在测试集上对比不同配置的效果
   - 优化后需要充分训练，让模型适应新的先验框分布
